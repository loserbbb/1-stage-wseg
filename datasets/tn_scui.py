...
class TNScuiSegmentation(TNScui):
    ...
    def __init__(self, cfg, split, test_mode, root=os.path.expanduser('./data')):
        ...
        if self.split == 'train':
            _split_f = os.path.join(self.root, 'train_tn.txt')
        # elif self.split == 'train_voc':
        #     _split_f = os.path.join(self.root, 'train_voc.txt')
        elif self.split == 'val':
            _split_f = os.path.join(self.root, 'val_tn.txt')
        elif self.split == 'test':
            _split_f = os.path.join(self.root, 'test_tn.txt')
        else:
        ...
        if self.split != 'test':
            ...
            if self.split == 'train':
                assert len(self.images) == 2875
            elif self.split == 'val':
                assert len(self.images) == 397
